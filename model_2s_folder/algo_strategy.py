...
def revamp_map(omap):  # have to edit internal values first
    if len(omap) <= 0:
        # return
        pass
    brq = omap[:14]
    for i in range(len(brq)):
        brq[i] = brq[i][14:]  # bottom right quadrant
    blq = omap[:14]
    for i in range(len(blq)):
        blq[i] = blq[i][:14]  # bottom left quadrant
    for i, obj in enumerate(brq):
        brq[i] = obj[: (len(obj) - i)]
    for i, obj in enumerate(blq):
        blq[i] = obj[(len(obj) - i) :]
        blq[i].append(brq[-i])
    bottom_half = blq
    ...
    for i in range(len(trq)):
        trq[i] = trq[i][14:]  # top right quadrant
    tlq = omap[14:]
    for i in range(len(tlq)):
        tlq[i] = tlq[i][:14]
    for i, obj in enumerate(trq):
        trq[i] = obj[:i]
    for i, obj in enumerate(tlq):
        tlq[i] = obj[i:]
        tlq[i].append(trq[-i])
    top_half = tlq
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_turn(self, turn_state):
        ...
        the_game_map = game_state.game_map.get_this_map()
        for row_idx, row in enumerate(the_game_map):
            for position_idx, position in enumerate(row):
                for unit in position:
                    # with open("tempfile.txt.", "a") as f:
                    # f.write(str(str(unit) + "\n" + str(type(unit)) + "\n"))
                    ff, ef, df, pi, ei, si = 0, 0, 0, 0, 0, 0
                    if unit.player_index == 0:
                        ...
                        if unit.unit_type == "PI":
                            pi += 1
                        if unit.unit_type == "EI":
                            ei += 1
                        if unit.unit_type == "SI":
                            si += 1
                    ff1, ef1, df1, pi1, ei1, si1 = 0, 0, 0, 0, 0, 0
                    if unit.player_index == 1:
                        ...
                        if unit.unit_type == "SI":
                            si1 += 1
                    the_game_map[row_idx][position_idx] = [
                        ff,
                        ef,
                        df,
                        pi,
                ...
                if len(position) <= 0:
                    the_game_map[row_idx][position_idx] = [0] * 12

        with open("mapfile.txt", "a") as f:
            f.write(str(len(revamp_map(the_game_map)[0][0])))
