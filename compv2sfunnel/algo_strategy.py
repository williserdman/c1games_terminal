...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def attack(self, game_state):
        ...
        options = [
            [2, 13],  # upper left wall
            [7, 11],  # middle vertical wall
            [17, 8],  # middle horizontal wall
            [22, 10],  # funnel
        ]

        if (
            game_state.get_resource(MP) > 8 + 4 * (game_state.turn_number % 10)
            and not self.attack_in_progress
        ) or self.turns_inactive >= 6:
            # self.scouts = random.randint(0, 1)
            self.scouts = True

            if self.scouts:
                best_location = self.least_damage_spawn_location(game_state, options)
                # self.attack_route = options.index(best_location)
                # game_state.attempt_remove(best_location)
                self.attack_route = 3
                self.attack_in_progress = True
                self.scouts = True
            else:
                best_location = self.most_destructive_spawn_option(game_state, options)
                # self.attack_route = options.index(best_location)
                # game_state.attempt_remove(best_location)
                self.attack_route = 3
                self.attack_in_progress = True
    ...
    def build_defences(self, game_state):
        ...
        # self.step6(game_state)
        # self.step7(game_state)
        self.dumb(game_state)

    def dumb(self, game_state):
        locations = [
            [4, 9],
            [5, 9],
            [5, 8],
    ...
    def step3d(self, game_state):
        ...
        if self.attack_route != MID_VERTICAL:
            game_state.attempt_spawn(WALL, [7, 11])
        if self.attack_route != MID_HORIZONTAL:
            game_state.attempt_spawn(WALL, [17, 8])
