...
# keywords: note, todo, test, urgent
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        self.supports_done = False
        self.do_i_attack = False
    ...
    def strategy(self, game_state):
        ...
        if self.corner_bool:
            self.corner_scout_attack(
                game_state
            )  # todo: split up build_defences for easier turn based strat
        ...
        if (
            ...
            or not self.do_i_attack  # todo: don't need to stall if everything can be built
        ):
        ...
        elif game_state.get_resource(MP) < (10 * game_state.turn_number % 20):
            pass
        else:
            ...
            if game_state.get_resource(SP) > 0:
                self.supports_done = True
            else:
                self.supports_done = False
            game_state.attempt_upgrade(
                support_locations
            )  # urgent, sell/remove supports if we don't have enough for our original defences https://terminal.c1games.com/watchLive/10318455
    ...
    def build_defences(self, game_state):
        ...
        if game_state.get_resource(SP) > 1:
            self.do_i_attack = True
        else:
            self.do_i_attack = False

        game_state.attempt_upgrade(upgrades)

        if self.supports_done:
            game_state.attempt_upgrade(
                turret_locations
                + attack_turrets
                + first_walls
    ...
    def corner_scout_attack(self, game_state):
        if self.corner_bool:
            ...
            game_state.attempt_spawn(
                INTERCEPTOR, [24, 10], round(game_state.get_resource(MP) / 3)
            )
    ...
    def replace_damaged_walls(self, game_state):
        ...
        for location in game_state.game_map:
            if game_state.contains_stationary_unit(location):
                for unit in game_state.game_map[location]:
                    if (
                        unit.player_index == 0 and unit.health != unit.max_health
