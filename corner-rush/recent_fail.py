...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def should_i_mid_scouts(self, game_state):
        gamelib.debug_write(game_state.game_map[27, 14])
        for unit in game_state.game_map[27, 14]:
            if unit.stationary:
                pass
        h = 60  # unit.health urgent todo: doens't work when someone is constantly replacing corners
        ...
        if self.first_group + sacrifices > 6 * math.ceil(game_state.turn_number % 10):
            return True
        else:
            return False
        return False
    ...
    def strategy(self, game_state):
        ...
        if game_state.get_resource(SP) > 10:
            self.replace_damaged_walls(
                game_state
            )  # note: only doing this when we are beginning to accumulate SP

        # If the turn is less than 5, stall with interceptors and wait to see enemy's base
        if (
            game_state.turn_number < 5
            or game_state.get_resource(SP)
            < 2  # todo: don't need to stall if everything can be built
        ):
            # todo: this line is a hotfix for another task. go find it :) something about stalling with interceptors to accumulate SP
            self.stall_with_interceptors(game_state)
        elif game_state.get_resource(MP) < (9 * math.ceil(game_state.turn_number % 10)):
            pass
            # todo: could stall with 1 interceptor
        else:
            ...
            # If they have their attack line at y = 16
                ...
                gamelib.debug_write("here?")
                if not self.should_i_mid_scouts(game_state):
                    ...
                    for i in range(28):
                        temp.append([i, 13])
                    x, d = self.least_damage_spawn_location(game_state, temp)
                    x = x[0]
                    if d / 15 > 4 * math.ceil(game_state.turn_number % 10):
                        pass
                    else:
                        pass  # todo """
