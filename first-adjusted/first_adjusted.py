...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def attack(self, game_state):
        if self.active:
            ...
            choice, dmg = self.least_damage_spawn_location(game_state, spawn_options)
            num_of_supports = 0
            x_adj = 0

            index = spawn_options.index(choice)

            if index:  # this will run for [14, 0]
                for s in supports:
                    x = 13 - (s[0] - 14)
                    num_of_supports += game_state.attempt_spawn(SUPPORT, [x, s[1]])
                    game_state.attempt_remove([x, s[1]])
                x_adj = -1
            else:
                num_of_supports = game_state.attempt_spawn(SUPPORT, supports)
                game_state.attempt_remove(supports)
                x_adj = 1

            sacrifices = round(dmg / (15 + 4 * num_of_supports))

            enemy_edges = game_state.game_map.get_edge_locations(
                game_state.game_map.TOP_LEFT
            ) + game_state.game_map.get_edge_locations(game_state.game_map.TOP_RIGHT)

            if game_state.find_path_to_edge(choice)[-1] not in enemy_edges:
                game_state.attempt_spawn(DEMOLISHER, choice, 1000)
            else:
                game_state.attempt_spawn(
                    SCOUT,
                    [choice[0] + x_adj, choice[1] + 1],
                    sacrifices + 5,
                )
                game_state.attempt_spawn(SCOUT, choice, 1000)
            ...
            temp = others_mp < 6
            gamelib.debug_write(others_mp, temp)
            logic = (
                (temp or self.turns_inactive >= 6) and temp
            ) or self.turns_inactive >= 8
            if logic:
                self.active = True
