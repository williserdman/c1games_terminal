...
class GameUnit:
    ...
    def __init__(self, unit_type, config, player_index=None, health=None, x=-1, y=-1):
        ...
        self.lvl1health = self.max_health
    ...
    def upgrade(self):
        temp = copy.copy(self.max_health)
        self.__upgrade()
        if self.max_health != temp:
            self.health += self.max_health - self.lvl1health

    def __upgrade(self):
        from .game_state import UNIT_TYPE_TO_INDEX

        type_config = self.config["unitInformation"][
            UNIT_TYPE_TO_INDEX[self.unit_type]
        ].get("upgrade", {})
        ...
        self.cost = [
            type_config.get("cost1", 0) + self.cost[0],
            type_config.get("cost2", 0) + self.cost[1],
        ]
        self.upgraded = True
    ...
    def __toString(self):
        ...
        return "{} {}, health: {} location: {} removal: {} upgrade: {} ".format(
            owner, self.unit_type, self.health, [self.x, self.y], removal, self.upgraded
        )
